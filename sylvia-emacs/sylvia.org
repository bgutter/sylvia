...
(defun sylvia:--echo-phonemes-in-region ()
  ...
  (sylvia:poem-phonemes-in-region
    ...
    (sylvia:--echo-phonemes-in-region--deferred-generator
      (buffer-substring-no-properties (region-beginning) (region-end)))))

(defun sylvia:--echo-phonemes-in-region--deferred-generator (text)
  "Deferred callback generator for `sylvia:echo-phonemes-in-region'"
  (lexical-let
      ((captured-text text))
...
(defun sylvia:--echo-phonemes-at-point--deferred-generator (word)
  ...
  (lexical-let
    ...
    #'(lambda (phoneme-reprs)
        (when phoneme-reprs
          (sylvia:--message-no-log "%s: %s" captured-word phoneme-reprs)))))
#+END_SRC
...
choose the 'best' one. Returns a list of strings."
  (sylvia:poem-phonemes-in-region (region-beginning) (region-end)))
#+END_SRC
