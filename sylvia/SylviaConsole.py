...
class SylviaConsole(cmd.Cmd):
    ...
    def setConfig(self, *args, **kwargs):
        ...
        if len(args) == 0:
            if interactive:
                self.errorMessage(
                    "No configuration option given. Type 'help config' for more information."
                )
            else:
                assert False
        else:
            ...
            if config == "charwidth":
                if len(args) != 2:
                    if interactive:
                        self.errorMessage(
                            "charwidth argument needs to be a single integer."
                        )
                    else:
                        assert False
                else:
                ...
                if len(args) != 2:
                    if interactive:
                        self.errorMessage(
                            "inferunknown argument needs to be 0/1 or True/False."
                        )
                    else:
                        assert False
                else:
    ...
    def generateNextHandle(self):
        ...
        while True:
            name = "Poem" + str(i)
            if name not in list(self.poems.keys()):
                break
        return name
    ...
    def do_lregex(self, arg):
        ...
        if len(args) != 1:
            self.errorMessage(
                "Need a single argument -- the regular expression for which to match."
            )
        else:
    ...
    def do_popularity(self, arg):
        ...
        if len(args) != 1:
            self.errorMessage(
                "Bad number of arguments. Type 'help popularity' for details."
            )
        else:
    ...
    def do_compose(self, arg):
        ...
        if len(args) > 1:
            self.errorMessage(
                "Bad number of arguments. Send the handle or nothing at all."
            )
        else:
            if len(args) == 1:
                handle = args[0]
    ...
    def do_load(self, arg):
        ...
        if not 0 < len(args) < 3:
            self.errorMessage(
                "Bad number of arguments. Type 'help load' for more info."
            )
        else:
            if len(args) == 2:
                handle = args[1]
    ...
    def do_show(self, arg):
        ...
        if len(args) != 1:
            self.errorMessage(
                "Need exactly one argument: the name of the poem to show."
            )
        else:
            handle = args[0]
    ...
    def do_test_infer(self, arg):
        ...
        if len(args) > 1:
            self.errorMessage(
                "test_infer takes 0 or 1 args: the optional number of words to check."
            )
            return
        if len(args) == 1:
            count = int(args[0])
        ...
        hits = 0
        for word in sorted(
            list(self.pd.entries.keys()), key=lambda x: -self.pd.findPopularity(x)
        )[:count]:
