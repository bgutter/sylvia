...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o",
        "--optimize_dictionary",
        help="Compile the dictionary to speed up later queries.",
    )
    parser.add_argument(
        "-d", "--dictionary_path", help="Point to an alternate dictionary file."
    )
    parser.add_argument(
        "-w", "--popularity_path", help="Point to an alternate popularity file."
    )
    parser.add_argument("-c", "--command", help="Run a one-off command.")
    parser.add_argument(
        "-e", "--emacs_server", action="store_true", help="Start Emacs RPC Server"
    )
    ...
    if args.dictionary_path:
        if os.path.splitext(args.dictionary_path)[1].upper() == ".TXT":
            ...
            if not args.popularity_path:
                print(
                    "If using a text dictionary file, a popularity file is also needed."
                )
                exit()
